function generate_imagelist(root)
    ...
    for i in 1:N
        ...
        table *= "| ![]($(path_thumbnail)) | $(name) | `$(color)` | `$(size)` | $(note) |\n"
    end

    comment = "[This part will be automatically generated by `docs/generate_imagelist.jl`.]"
    script = read(joinpath(root, "imagelist.md"), String)
    write(joinpath(root, "imagelist.md"), replace(script, comment=>table))
end
